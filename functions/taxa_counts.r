...
#print(headers)
#write.table(headers,file = paste0(opt$out_path, opt$sample_name, 'headers.txt'),quote = F, sep='\t', row.names = F, col.names = T)
...
#write.table(s.bc, file = paste0(opt$out_path, opt$sample_name, '.all.barcodes_umi.txt'),
...
s.bc$barcode_factor <- factor(s.bc$barcode)
s.bc$taxid_factor <- factor(s.bc$taxid)

s.mat <- sparseMatrix(i = as.integer(s.bc$barcode_factor), 
                      j = as.integer(s.bc$taxid_factor), 
                      x = s.bc$umi)

#print(s.mat)
#s.mat = sparseMatrix(as.character(s.bc$barcode), as.integer(s.bc$taxid), x=s.bc$umi)
colnames(s.mat) = levels(factor(s.bc$taxid))
rownames(s.mat) = levels(factor(s.bc$barcode))
s.mat = t(s.mat)
...
#print(df)
#write.table(df, file = paste0(opt$out_path, opt$sample_name, '.all.barcodes_taxa_rank.txt'),
#            quote = F, sep='\t', row.names = F, col.names = T)
df = df %>% 
