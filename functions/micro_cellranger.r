...
step7 = read.delim(argv[4], header = T)

test<-function(tax,cou){
co_210=cou %>% subset(taxid == tax)
c1=co_210[,c( 'barcode','counts' ,'taxid')]
Count_mtx1 <- acast( c1, barcode ~ taxid, value.var="counts" )
barcode_210=rownames(Count_mtx1)
t1 <- apply( Count_mtx1, 1, function(x) { return( sum( x, na.rm=TRUE ) ); } ) 
result_210=data.frame(barcode_210,umi_210=t1[ barcode_210 ])


c1=cou[which(cou$rank == 'k'),][,c( 'barcode','counts' ,'taxid')]
Count_mtx1 <- acast( c1, barcode ~ taxid, value.var="counts" )

barcode=rownames(Count_mtx1)
t1 <- apply( Count_mtx1, 1, function(x) { return( sum( x, na.rm=TRUE ) ); } ) 
result=data.frame(barcode,TotalUMIs=t1[ barcode ])
########################
...
res=lapply( cell$taxid , function(x) { return( test(x,step7) ); } )
